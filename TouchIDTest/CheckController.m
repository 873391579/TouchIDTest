...
-(BOOL)checkPassword:(NSString*)pdw{
   ...
   NSArray *userDic = [queue queueQueryDataFromTableForm:@"userData" keyAndValue:@{@"name":name}];
    if (userDic.count>0 && [[[userDic firstObject] objectForKey:@"password"] isEqualToString:pdw] ) {
        return YES;
    }
    return NO;
}
...
- (void)saveSettings:(BOOL)sender
{
    ...
    NSString *name = [[NSUserDefaults standardUserDefaults] objectForKey:@"name"];
    NSArray *userDic = [queue queueQueryDataFromTableForm:@"userData" keyAndValue:@{@"name":name}];
    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:[userDic firstObject]];
    [dic setObject:[NSNumber numberWithBool:sender] forKey:@"touchID"];
    [queue queueUpdate:dic where:@{@"name":name} fromTable:@"userData"];
}
